    ...
    <div className="bg-[#0d0d0d] text-white min-h-screen">
      ...
      <main className="max-w-[1500px] mx-auto px-6 py-10 flex flex-col lg:flex-row gap-10">
        ...
        <section className="flex-1 max-w-7xl">
          <h2 className="font-semibold text-white text-lg mb-6 select-none">My Cart</h2>
          <div className="bg-[#1a1a1a] rounded-lg p-4 space-y-4 text-xs text-gray-300">
            ...
            {cartItems.map((item) => (
              <div key={item.id} className="flex bg-[#222222] rounded-lg p-3 gap-4 relative items-center">
                <img src={item.image} alt={item.title} className="w-[90px] h-[120px] object-cover rounded-sm flex-shrink-0" />
                <div className="flex-1 flex flex-col justify-between min-w-0">
                  <div>
                    <div className="flex justify-between items-start gap-2">
                      <h3 className="flex-1 min-w-0 text-white font-semibold text-sm leading-tight line-clamp-3">{item.title}</h3>
                      <span className="bg-[#5539cc] px-2 py-0.5 rounded font-medium text-white mr-5">GAME</span>
                    </div>
                    {/* <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-[#6a6aff] uppercase">GAME</span>
                      </div> */}
                    <span className="text-[#0099ff] text-xs font-semibold uppercase mt-1 inline-block">GLOBAL</span>
                  </div>
                ...
                <div className="absolute bottom-9 right-8 text-white font-semibold text-sm whitespace-nowrap">
        ...
        <section className="w-full lg:max-w-sm flex flex-col gap-6">
          <h2 className="font-semibold text-white text-lg select-none">Summary</h2>
          <div className="bg-[#1a1a1a] rounded-lg p-4 space-y-3 text-xs text-gray-300">
            <div className="flex items-center gap-1 text-[#6a6aff] font-semibold uppercase text-[9px]">
              <span>plus</span>
              <span className="bg-[#6a6aff] rounded px-1 text-[7px] font-bold tracking-widest">MEMBERSHIP</span>
              <span>Save extra ₹107.71</span>
            </div>
            <div>
              <p className="text-gray-300 text-xs font-semibold mb-1">Your cart total</p>
              <p className="text-white font-extrabold text-2xl mb-1">₹{total}</p>
              <p className="text-gray-500 text-[9px] flex items-center gap-1">PRICE NOT FINAL <i className="fas fa-info-circle text-gray-500"></i></p>
            </div>
            <Link href="/checkout">
